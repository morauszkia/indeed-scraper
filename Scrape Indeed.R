...
while (TRUE) {
  for (i in 1:15) {
    ...
    xpath_expression_occupation <- sprintf("(//div[contains(@class, 'css-1m4cuuf e37uo190')])[%d]", i)
    ...
    xpath_expression_salary <- sprintf("(//div[contains(@class, 'heading6 tapItem-gutter metadataContainer noJEMChips salaryOnly')])[%d]", i)
...
# Final data 
all <-cbind(location, company, occupation, desc, rating, salary, date)
